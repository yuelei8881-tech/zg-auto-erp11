...
begin
  ...
  select * into v_invite from public.zg_staff_invites
  where lower(email) = v_email and status = 'pending'
    and (expires_at is null or expires_at > now())
  order by created_at desc limit 1;
  if v_invite.id is null then return null; end if;
  insert into public.zg_organization_members (organization_id,user_id,role,status,display_name,permissions)
  values (v_invite.organization_id,v_user,v_invite.role,'active',split_part(v_email,'@',1),'{}'::jsonb)
