...
const roles = [
  ...
  { value: 'workshop_supervisor', label: '车间主管' },
...
export function StaffPage({ cloud }: { cloud: CloudSession }) {
  ...
  const [email, setEmail] = useState(''); const [role, setRole] = useState('technician'); const [customRole, setCustomRole] = useState('');
  ...
  const roleOptions = [...roles, ...customRoles.map(value => ({ value, label: value }))];
  const permissionChecked = (member: StaffMember, key: string) => Object.prototype.hasOwnProperty.call(member.permissions, key) ? Boolean(member.permissions[key]) : (rolePermissionDefaults[member.role] || []).includes(key);
  const changeRole = async (member: StaffMember, nextRole: string) => {
    if (nextRole === '__custom__') {
      const name = prompt('请输入新的角色名称，例如：服务顾问、车队协调员');
      if (!name?.trim()) return;
      return update(member, { role: name.trim(), permissions: {} });
    }
    await update(member, { role: nextRole, permissions: {} });
  };
