...
function App({ cloud }: { cloud: CloudSession }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main"><header className="topbar"><div className="global-search">⌕<input value={searchDraft} onChange={e => setSearchDraft(e.target.value)} onKeyDown={e => e.key === 'Enter' && runGlobalSearch()} placeholder="搜索客户、电话、VIN、车牌、工单、司机…" /><button type="button" onClick={runGlobalSearch}>搜索</button>{searchSuggestions.length > 0 && <div className="search-suggestions">{searchSuggestions.map((item, index) => <button type="button" key={`${item.page}-${item.label}-${index}`} onClick={() => { setSearchDraft(item.query); setSearch(item.query); setPage(item.page); }}><b>{item.label}</b><small>{item.meta}</small></button>)}</div>}</div><div className="top-status"><span className={syncing ? 'syncing' : ''}>{syncing ? '正在同步…' : '● 云端已同步'}</span><span>{actorName}</span><b>v0.78.3</b><button type="button" className="topbar-logout" onClick={() => confirm('确定退出当前账号？') && void cloud.signOut()}>退出</button></div></header>
...
function SendMenu({ order, settings, store, cloud }: { order: WorkOrder; settings: ShopSettings; store: AppStore; cloud: CloudSession }) {
  ...
  const sendCustomerDocument = async (kind: string) => {
    ...
    const html = baseHtml.replace(/<\/div>$/, `${evidenceHtml}</div>`);
    const subject = `${settings.shopName} · ${title} · ${order.number}`;
    try {
