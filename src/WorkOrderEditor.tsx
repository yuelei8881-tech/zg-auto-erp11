...
type Props = {
  ...
  onCheckoutAndDeliver: (order: WorkOrder, paymentMethod: string) => Promise<WorkOrder | undefined>;
    ...
    canPrintPricedDocuments: boolean;
  cloud: CloudSession;
  currentUser: string;
...
export function WorkOrderEditor({ value, customers, vehicles, fleets, drivers, workOrders, parts, servicePackages, settings, nextNumber, onSave, onCancel, onCheckoutAndDeliver, onCreateVehicle, onSaveServicePackage, onDeleteServicePackage, onPrint, cloud, currentUser, currentUserId, technicians, canApproveReview, canAssignTechnician, canEditPricing, canViewFinancials, canCheckoutAndDeliver, canPrintDocuments, canPrintPricedDocuments }: Props) {
  ...
  const selectWorkflowStage = (stage: NonNullable<WorkOrder['workflowStage']>) => {
    const destination: Record<NonNullable<WorkOrder['workflowStage']>, MobileStep> = {
      ...
      '报价待确认': 'quote',
      '维修施工': 'repair',
      '完工待结账': canCheckoutAndDeliver ? 'checkout' : 'repair',
      '已结账': canCheckoutAndDeliver ? 'checkout' : 'repair',
    };
    selectMobileStep(destination[stage]);
  };

  return <div className="editor-screen focused-editor" data-panel={activePanel} data-mobile-step={mobileStep}>
    <div className="editor-head"><div><p className="eyebrow">维修工单 / Repair Order</p><h2>{value ? `编辑 ${order.number}` : '新建维修工单'}</h2>{value && <small>已明确选择此工单；只有点击“保存工单”或“保存进度”才会写入服务器。</small>}</div><div className="toolbar">{canPrintDocuments && <>{canViewFinancials && canPrintPricedDocuments && <button type="button" onClick={() => printCurrentOrder(false)}>打印带价格工单</button>}<button type="button" onClick={() => printCurrentOrder(true)}>内部施工单（无价格）</button></>}<button type="button" onClick={cancelEditor}>取消</button><button type="button" className="primary" onClick={submit} disabled={saving}>{saving ? '保存中…' : '保存工单'}</button></div></div>
    ...
    <section className="form-section"><div className="section-title"><div><h3>人工项目</h3><span className="muted">左右滚动选择常用维修项目，再根据实际情况修改工时和费率。</span></div><button onClick={addLabor}>＋ 自定义工时</button></div>
      ...
      {calculated.laborItems.filter(item => !item.linkedPartItemId).map(item => { const flat = item.billingMode === 'flat'; return <div className="line-row labor-grid" key={item.id}><input list="labor-price-history" value={item.description} onChange={e => updateLabor(item.id, { description: e.target.value })} onBlur={e => applyRememberedLaborPrice(item.id, e.target.value)} placeholder="例如：更换水泵（同名项目自动带入上次价格）" /><input type="number" inputMode="decimal" min="0" step="1" value={item.qty ?? 1} onChange={e => updateLabor(item.id, { qty: Number(e.target.value) })} aria-label="人工项目数量" placeholder="数量" /><select value={flat ? 'flat' : 'hourly'} disabled={!canEditPricing} onChange={e => updateLabor(item.id, { billingMode: e.target.value as 'hourly' | 'flat', flatAmount: e.target.value === 'flat' ? item.total / Math.max(1, Number(item.qty || 1)) : item.flatAmount })}><option value="hourly">按小时</option><option value="flat">一口价</option></select><input type="number" inputMode="decimal" step={flat ? '0.01' : '0.1'} value={canViewFinancials ? editableNumber(flat ? item.flatAmount : item.hours) : ''} placeholder={canViewFinancials ? '' : '隐藏'} disabled={!canEditPricing} onChange={e => updateLabor(item.id, flat ? { flatAmount: Number(e.target.value) } : { hours: Number(e.target.value) })} aria-label={flat ? '每件一口价金额' : '每件工时'} /><input type="number" inputMode="decimal" step="0.01" value={canViewFinancials ? editableNumber(item.rate) : ''} placeholder={canViewFinancials ? '' : '隐藏'} disabled={!canEditPricing || flat} onChange={e => updateLabor(item.id, { rate: Number(e.target.value) })} /><input value={item.technician || ''} onChange={e => updateLabor(item.id, { technician: e.target.value })} placeholder="技师" /><b>{canViewFinancials ? money(item.total) : '—'}</b><button className="danger-link" onClick={() => removeLabor(item.id)}>删除</button></div> })}
    ...
    {canViewFinancials && <section className="form-section totals-section"><div><div className="form-grid four compact">
      ...
      <label>实际结账金额（修改浮动 $10 以内免审批）<input type="number" step="0.01" placeholder={`自动计算 ${money(calculated.laborTotal + calculated.partsTotal + calculated.outsource + calculated.tax - calculated.discount)}`} value={order.settlementTotal ?? ''} onChange={e => patch({ settlementTotal: e.target.value === '' ? undefined : Number(e.target.value) })} /></label>
