...
export function printDocumentV077(order: WorkOrder, settings: ShopSettings, kind: string) {
  ...
  const laborRows = (order.laborItems || []).map(item => `<tr><td>${escapeHtml(item.description)}</td><td class="n">${Number(item.hours).toFixed(1)}</td><td class="n">${money(item.rate)}</td><td class="n">${money(item.total)}</td></tr>`).join('');
...
@page{size:Letter;margin:.28in}*{box-sizing:border-box}body{font-family:Arial,"Microsoft YaHei",sans-serif;font-size:7pt;color:#111;margin:0}.top{display:grid;grid-template-columns:1fr 1fr 1fr;align-items:start;font-size:7pt}.top .t{text-align:center;font-size:10pt}.top .ro{text-align:right;color:#d60000;font-size:14pt;font-weight:800}.brand{display:grid;grid-template-columns:220px 1fr;align-items:center;border-bottom:2px solid #111;padding:7px 0 8px}.brand svg{width:205px;height:81px;margin:0}.shop{text-align:center}.shop h1{font-size:22pt;letter-spacing:2px;margin:0 0 5px}.shop p{font-size:8pt;margin:0}.info{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-top:10px}.box{border:1px solid #777;min-height:58px;padding:5px;line-height:1.35}.box b{display:inline-block;width:57px}.section{margin-top:7px}.section-title{font-size:7pt;color:#666;margin:0 0 3px 3px}.text-box{border:1px solid #888;min-height:34px;padding:5px;white-space:pre-wrap}.line-title{font-weight:700;margin:6px 0 2px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #888;padding:2.5px 4px;vertical-align:top}th{font-weight:700;text-align:left;background:#f5f5f5}.n{text-align:right}.totals{width:46%;margin:7px 0 0 auto}.totals td:first-child{text-align:right}.total td{border-top:2px solid #111;font-size:9pt;font-weight:800}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:34px;margin-top:25px}.sig{border-top:1px solid #111;padding-top:3px;min-height:25px}.signed img{display:block;max-width:210px;max-height:52px;object-fit:contain}.signed span{font-size:6pt}.footer{text-align:center;margin-top:8px;color:#555;font-size:6pt}@media print{button{display:none}}
</style></head><body>
<div class="top"><div>${escapeHtml(now)}</div><div class="t">${escapeHtml(title)}</div><div class="ro">${escapeHtml(order.number)}</div></div>
...
<div class="section"><div class="section-title">Customer Concern / 客户描述</div><div class="text-box">${escapeHtml(order.complaint)}</div></div>
<div class="section"><div class="section-title">Diagnosis &amp; Work Performed / 诊断与维修</div><div class="text-box">${escapeHtml([order.diagnosis, order.workPerformed].filter(Boolean).join('\n'))}</div></div>
