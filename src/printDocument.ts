...
export function printDocumentV077(
  order: WorkOrder,
  ...
  const paymentRows = hidePrices ? '' : payments.filter(item =>
...
<div class="info"><div class="box"><b>Customer</b>${escapeHtml(order.customer)}<br><b>Phone</b>${escapeHtml(order.phone)}<br><b>Company</b>${escapeHtml(order.company || '')}<br><b>Driver</b>${escapeHtml(order.driver || '')} ${escapeHtml(order.driverPhone || '')}</div><div class="box"><b>Vehicle</b>${escapeHtml(order.vehicle)}<br><b>Plate</b>${escapeHtml(order.plate)}<br><b>VIN</b>${escapeHtml(order.vin)}<br><b>Mileage</b>${escapeHtml(order.mileage)} &nbsp;&nbsp; <b>PO</b>${escapeHtml(order.po || '')}</div></div><div class="internal-note">INTERNAL WORK SHEET / 内部施工单 · PRICE HIDDEN / 价格已隐藏</div>
